...
async function renderWiki(content) {
  let html = `
    <div class="section">
      <div class="section-title">Wiki & Knowledge Base</div>
      <div style="display:flex;gap:8px;margin-bottom:8px;">
        <button class="btn secondary" style="flex:1" id="wiki-local-btn" onclick="switchWikiSource('local')">Local Wiki</button>
        <button class="btn secondary" style="flex:1" id="wiki-outline-btn" onclick="switchWikiSource('outline')">Outline</button>
      </div>
      <input id="wiki-search" class="search-bar" placeholder="Search wiki..." oninput="debounceWikiSearch()">
    </div>
    <div id="wiki-content">${loading('Loading...')}</div>
  `;
...
function switchWikiSource(source) {
  _wikiSource = source;
  document.getElementById('wiki-local-btn').style.opacity = source === 'local' ? '1' : '0.5';
  document.getElementById('wiki-outline-btn').style.opacity = source === 'outline' ? '1' : '0.5';
  loadWikiContent();
}
  ...
  try {
    if (search) {
      ...
      if (data.collections && data.collections.length > 0) {
        ...
        container.innerHTML = `<div class="card"><p style="color:var(--hint)">${data.note || 'No collections found'}</p></div>`;
      }
    }
  } catch (err) {
...
async function readCollectionDoc(collection, docPath) {
  ...
  try {
    ...
    // Render as preformatted markdown
    content.innerHTML = `
      <div class="detail-back" onclick="popView()">← Back</div>
      <div class="card" style="font-family:monospace;font-size:13px;line-height:1.6;white-space:pre-wrap;overflow-x:auto;">${escapeHtml(data.content)}</div>
    `;
  } catch (err) {
    content.innerHTML = `<div class="card"><p style="color:var(--destructive)">${err.message}</p></div>`;
  }
...
async function readLocalFile(fullPath, displayName) {
  ...
  try {
    ...
    content.innerHTML = `
      <div class="detail-back" onclick="popView()">← Back</div>
      <div class="card" style="font-family:monospace;font-size:13px;line-height:1.6;white-space:pre-wrap;overflow-x:auto;">${escapeHtml(data.content || 'Unable to read file')}</div>
    `;
  } catch (err) {
    content.innerHTML = `<div class="card"><p style="color:var(--destructive)">${err.message}</p></div>`;
  }
